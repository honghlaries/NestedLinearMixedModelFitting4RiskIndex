...
datareadln <- function() { ## data readln ----
    ...
    dplyr::select(NCPI_t = NemerowCPI_10,NCPI_e = NemerowCPI_12,SiteID,SplMonth,group) %>% 
    tidyr::gather(elem, resp, NCPI_t:NCPI_e)
}
...
meanseCal <- function(dat) { ## calulate and output mean and se
  ...
  dat %>%
    ...
    write.csv("ncpi/log/IndexStat.csv")
}
...
multiElementMod <- function(dat, fact, SplMonthlv, grouplv, glv, gcode, 
  ...
  ## output
  if (log) {write.csv(x = fe.g, file = paste("ncpi/log/FixedEff",suffix,".csv",sep = ""), row.names = F)
    write.csv(x = re.g, file = paste("ncpi/log/RandomEff",suffix,".csv",sep = ""), row.names = F)
    write.csv(x = modinf.g, file = paste("ncpi/log/ModelChoice",suffix,".csv",sep = ""), row.names = F) 
    write.csv(x = shapiro.g, file = paste("ncpi/log/ShapiroRawData",suffix,".csv",sep = ""), row.names = F) 
    write.csv(x = posthoc.g, file = paste("ncpi/log/Posthoc",suffix,".csv",sep = ""), row.names = F) 
    write.csv(x = modavo.g, file = paste("ncpi/log/ModelAnova",suffix,".csv",sep = ""), row.names = F) 
...
## Gather ploting ----
ggsave(plot = plotFEsim2facet(read.csv("ncpi/log/FixedEff.csv") %>% 
                                filter(term != "(Intercept)") %>% 
                                mutate(term = gsub("group","",term)) %>% 
                                mutate(term = gsub("SplMonth","",term)),
                              glv = c("EA","WE", "NV"), gcode = c("#31B404","#013ADF","grey50"), ncol = 2,
                              theme = theme_bw() + theme(legend.position = "none",
                                                         axis.text = element_text(size= 4,angle = 30),
                                                         axis.title = element_text(size= 6),
                                                         strip.text = element_text(size= 6))), 
       "ncpi/plot/Fixeff.png",
       width = 3, height = 2, dpi = 600)

## xy plot
theme_xy <- theme_bw() + theme(legend.position = "right")
ggsave(plot = ggplot(data = read.csv("./Data/Result_Sediment.csv") %>%
                       dplyr::inner_join(read.csv("./Data/meta_SedimentSampleList.csv"), by = c("SplNo" = "SplNo")) %>%
                       dplyr::right_join(read.csv("./Data/meta_Quadrat.csv"), by = c("QudNo" = "QudNo")) %>%
                       dplyr::inner_join(read.csv("./Data/meta_SiteGroup.csv"), by = c("SiteID" = "SiteID")) %>% 
                       dplyr::select(NCPI_t = NemerowCPI_10,NCPI_e = NemerowCPI_12,SiteID,SplMonth,group)) +
